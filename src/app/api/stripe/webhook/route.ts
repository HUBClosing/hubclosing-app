...
* → Rétrograder à free avec les crédits du tier free (3 déblocages).
*/
...
// Rétrograder à free — donner les crédits du tier free (3 déblocages)
...
recruiter_annonces_remaining: 'annonces' in freeLimits ? freeLimits.annonces : 0,
recruiter_deblocages_remaining: 'deblocages_included' in freeLimits ? freeLimits.deblocages_included : 0,
recruiter_boosts_remaining: 'boosts_included' in freeLimits ? freeLimits.boosts_included : 0,
updated_at: new Date().toISOString(),
}).eq('id', user.id);

console.log(`❌ Subscription deleted: user=${user.id} → free (credits reset to free tier)`);
break;
}
