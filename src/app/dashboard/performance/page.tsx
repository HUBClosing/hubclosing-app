...
export default async function PerformancePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── UpgradeGate : cv_performance requis (starter+) ── */}
      <UpgradeGate user={user} requiredFeature="cv_performance">
        <div className="space-y-6">
          {/* Actions */}
          ...
          {hasEntries && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatsCard title="Revenu total closé" value={formatCurrency(totalRevenue)} icon={<DollarSign className="h-6 w-6" />} />
              <StatsCard title="Deals closés" value={totalDeals} icon={<Handshake className="h-6 w-6" />} />
              <StatsCard title="Cash / appel moyen" value={formatCurrency(avgCashPerCall)} icon={<Phone className="h-6 w-6" />} />
              <StatsCard title="Conversion moyenne" value={`${avgConversion.toFixed(1)}%`} icon={<TrendingUp className="h-6 w-6" />} />
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-brand-dark flex items-center gap-2">
                <Briefcase className="h-5 w-5 text-brand-amber" /> Mes offres
              </h2>
              <Link
                  ...
                  <Card key={entry.id}>
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        {entry.product_price && (
                          <span className="text-sm text-gray-500">Produit : {formatCurrency(entry.product_price)}</span>
                        )}
                      </div>

                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                        <div><p className="text-xs text-gray-500">Revenu closé</p><p className="text-lg font-bold text-brand-dark">{formatCurrency(entry.revenue_closed)}</p></div>
                        <div><p className="text-xs text-gray-500">Appels</p><p className="text-lg font-bold text-brand-dark">{entry.calls_made}</p></div>
                        <div><p className="text-xs text-gray-500">Deals closés</p><p className="text-lg font-bold text-brand-dark">{entry.deals_closed}</p></div>
                        <div><p className="text-xs text-gray-500">Cash / appel</p><p className="text-lg font-bold text-brand-dark">{formatCurrency(entry.cash_per_call)}</p></div>
                        <div><p className="text-xs text-gray-500">Conversion brute</p><p className="text-lg font-bold text-brand-dark">{entry.conversion_rate_gross != null ? `${entry.conversion_rate_gross}%` : '—'}</p></div>
                        <div><p className="text-xs text-gray-500">Conversion nette</p><p className="text-lg font-bold text-brand-dark">{entry.conversion_rate_net != null ? `${entry.conversion_rate_net}%` : '—'}</p></div>
                      </div>
              ...
              <Card>
                <CardContent>
                  <EmptyState
                    ...
                    description="Ajoutez vos performances par offre pour construire votre CV de closer."
                    action={
                      <Link href="/dashboard/performance/add-entry" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-brand-green rounded-lg hover:bg-brand-dark transition-colors">
                        <Plus className="h-4 w-4" /> Ajouter ma première offre
                      </Link>
                    }
                  />
                </CardContent>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-brand-dark flex items-center gap-2">
                <Video className="h-5 w-5 text-brand-amber" /> Mes vidéos
              </h2>
              <Link
                      ...
                      <a href={video.url} target="_blank" rel="noopener noreferrer" className="block">
                        <div className="relative aspect-video bg-gray-100 rounded-t-xl overflow-hidden">
                          {thumbnail ? (
                            ...
                            <div className="flex items-center justify-center h-full"><Video className="h-10 w-10 text-gray-400" /></div>
                          )}
                        </div>
                        <CardContent className="p-4">
                          <div className="flex items-center gap-2 mb-1">
                            <Badge variant="default">{videoTypeLabels[video.video_type] || video.video_type}</Badge>
                          ...
                          {video.offer_name && <p className="text-xs text-gray-400 mt-2">Offre : {video.offer_name}</p>}
                        </CardContent>
                      </a>
                    </Card>
                  );
              ...
              <Card>
                <CardContent>
                  <EmptyState
                    ...
                    description="Ajoutez des vidéos de présentation ou témoignages pour renforcer votre profil."
                    action={
                      <Link href="/dashboard/performance/add-video" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-brand-green rounded-lg hover:bg-brand-dark transition-colors">
                        <Plus className="h-4 w-4" /> Ajouter ma première vidéo
                      </Link>
                    }
                  />
                </CardContent>
